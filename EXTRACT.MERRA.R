...
wd <- "/Users/Diana/Desktop/STATISTIC DOWNSCALING /MERRA raw/variable set 1"
setwd(wd)
...
temp <- open.ncdf("MERRA100.prod.assim.tavg1_2d_slv_Nx.19790101.SUB.nc")
print(temp)
...
ext.MERRA <- function(filenames,dates){
 ...
 for (vname in varnames){
    var.value <- get.var.ncdf(data,vname)
...
dates<-seq(as.Date("1979/01/01"),as.Date("1979/12/31"),by="day")
# format the date
dates <- lapply(dates,format,format="%Y%m%d")
vardata <- ext.MERRA("MERRA100.prod.assim.tavg1_2d_slv_Nx",dates)
